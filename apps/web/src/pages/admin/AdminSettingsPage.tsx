...
export const AdminSettingsPage: React.FC = () => {
  ...
  const [sheinEnabled, setSheinEnabled] = useState(true);
  const [sheinShippingFee, setSheinShippingFee] = useState('100');
  const [sheinServiceFee, setSheinServiceFee] = useState('75');
  ...
  const fetchSettings = () => {
    ...
    settingsApi.getAll().then((data) => {
      data.forEach((s) => {
        ...
        if (s.key === 'shein_enabled') setSheinEnabled(s.value !== 'false');
        if (s.key === 'shein_shipping_fee') setSheinShippingFee(s.value);
        if (s.key === 'shein_service_fee') setSheinServiceFee(s.value);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await Promise.all([
        ...
        settingsApi.update('shein_enabled', sheinEnabled ? 'true' : 'false', 'GENERAL'),
        settingsApi.update('shein_shipping_fee', sheinShippingFee, 'GENERAL'),
        settingsApi.update('shein_service_fee', sheinServiceFee, 'GENERAL'),
  ...
  return (
    <div className="space-y-8 text-start max-w-4xl mx-auto pb-20">
      ...
      {backupMsg && (
        <div
          className={`p-3.5 rounded-xl text-xs font-bold flex items-center gap-2 ${backupMsg.type === 'success'
      ...
      <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(
              ...
              return (
                <div
                  key={item.key}
                  className={`p-4 rounded-2xl border transition-all duration-200 ${isEnabled
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-3 mb-3">
                    ...
                    <button
                      ...
                      className={`inline-flex items-center gap-2 px-2.5 py-1 rounded-xl text-[11px] font-bold transition-all border select-none ${isEnabled
                    ...
                    >
                      <span
                        className={`w-2 h-2 rounded-full ${isEnabled ? 'bg-black' : 'bg-zinc-400'
                          }`}
                      />
                      <span>
                        {isEnabled
        ...
        {/* ========================================================
            SHEIN CONCIERGE DYNAMIC PRICING & CONFIGURATION
        ======================================================== */}
        <Card className="p-6 space-y-6 border-zinc-200 dark:border-zinc-800">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-zinc-100 dark:border-zinc-800">
            <div className="flex items-center space-x-2.5 rtl:space-x-reverse">
              ...
              <div>
                <h3 className="text-sm font-black text-zinc-900 dark:text-zinc-100">
                  {isArabic ? 'إعدادات خدمة طلبات SHEIN والتسعير' : 'SHEIN Concierge Pricing & Config'}
                </h3>
                <p className="text-[11px] text-zinc-500">
                  {isArabic
                    ? 'التحكم في تفعيل الخدمة، رسوم الشحن الدولي، العمولة، والتوصيل الداخلي بالجنيه المصري'
                    : 'Manage service toggle, shipping fees, commission, and local delivery in EGP'}
                </p>
              </div>
            </div>
            ...
            <button
              type="button"
              onClick={() => setSheinEnabled(!sheinEnabled)}
              className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold transition-all border ${sheinEnabled
            ...
            >
              <span className={`w-2 h-2 rounded-full ${sheinEnabled ? 'bg-emerald-400' : 'bg-zinc-400'}`} />
              <span>{sheinEnabled ? (isArabic ? 'الخدمة مفعلة' : 'Service Active') : (isArabic ? 'الخدمة معطلة' : 'Service Inactive')}</span>
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs font-bold text-zinc-700 dark:text-zinc-300 mb-1">
                {isArabic ? 'الشحن الدولي من SHEIN (جنيه)' : 'SHEIN International Shipping (EGP)'}
              </label>
              <Input
                type="number"
                min={0}
                value={sheinShippingFee}
                onChange={(e) => setSheinShippingFee(e.target.value)}
                placeholder="100"
              />
            </div>
            ...
            <div>
              <label className="block text-xs font-bold text-zinc-700 dark:text-zinc-300 mb-1">
                {isArabic ? 'رسوم الخدمة والتخليص (جنيه)' : 'Service / Handling Fee (EGP)'}
              </label>
              <Input
                type="number"
                min={0}
                value={sheinServiceFee}
                onChange={(e) => setSheinServiceFee(e.target.value)}
                placeholder="75"
              />
            </div>
            ...
            <div>
              <label className="block text-xs font-bold text-zinc-700 dark:text-zinc-300 mb-1">
                {isArabic ? 'التوصيل الداخلي بمصر (جنيه)' : 'Domestic Delivery (EGP)'}
              </label>
              <Input
                type="number"
                min={0}
                value={sheinDeliveryFee}
                onChange={(e) => setSheinDeliveryFee(e.target.value)}
                placeholder="60"
              />
            </div>
            ...
            <div>
              <label className="block text-xs font-bold text-zinc-700 dark:text-zinc-300 mb-1">
                {isArabic ? 'معامل تحويل العملة / الصرف' : 'Currency Exchange Multiplier'}
              </label>
              <Input
                type="number"
                step="0.1"
                min={0.1}
                value={sheinExchangeRate}
                onChange={(e) => setSheinExchangeRate(e.target.value)}
                placeholder="1"
              />
              <span className="text-[10px] text-zinc-400 mt-0.5 block">
                {isArabic ? 'اتركه 1 إذا كانت أسعار الرابط بالجنيه' : 'Keep 1 if link prices are in EGP'}
              </span>
            </div>

            <div className="sm:col-span-2">
              <label className="block text-xs font-bold text-zinc-700 dark:text-zinc-300 mb-1">
                {isArabic ? 'مدة التوصيل التقديرية للعميل' : 'Estimated Delivery Time'}
              </label>
              <Input
                value={sheinEstimatedDays}
