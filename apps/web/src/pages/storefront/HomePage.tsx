...
export const HomePage: React.FC = () => {
  ...
  const heroSection = cmsSections.find((s) => s.type === 'HERO' || s.key === 'hero_banner');
  const promoSection = cmsSections.find((s) => (s.type === 'PROMO_BANNER' || s.key === 'promo_summer') && s.isActive);
  const aboutSection = cmsSections.find((s) => (s.type === 'ABOUT' || s.key === 'about_craft') && s.isActive);
  ...
  const heroBadge = heroPayload.badgeAr
    ? isArabic
      ? String(heroPayload.badgeAr)
      : String(heroPayload.badgeEn || heroPayload.badgeAr)
    : isArabic
      ? 'تشكيلة جديدة'
      : 'NEW DROP';

  const heroCtaText = heroPayload.ctaTextAr
    ? isArabic
      ? String(heroPayload.ctaTextAr)
  ...
  return (
    <div className="space-y-10 sm:space-y-16 lg:space-y-20 pb-16">
        ...
        <section className="space-y-4 sm:space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-lg sm:text-xl font-black text-zinc-900 dark:text-zinc-100">
              {isArabic ? 'تصفح حسب القسم' : 'Shop by Category'}
            </h2>
            <Link
              to="/shop"
      ...
      {/* ========================================================
          4. FEATURED PRODUCTS (User's Real Products)
      ======================================================== */}
      ...
      {promoSection && (
        <section className="p-8 sm:p-12 rounded-3xl bg-[#f6f5f1] dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 text-center space-y-4">
          <span className="text-[10px] font-extrabold uppercase tracking-widest text-amber-600 dark:text-amber-400">
            {isArabic ? 'عرض خاص' : 'SPECIAL PROMOTION'}
          </span>
