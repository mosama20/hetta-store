...
13. [سجل أحدث التحديثات والميزات (Recent Releases)](#-سجل-أحدث-التحديثات-والميزات-recent-releases--upgrades)
...
- **سكشن الأقسام بنمط الدوائر التفاعلية (Stories Style)**: عرض الأقسام في شريط أفقي انسيابي يحاكي قصص إنستغرام مع تدرجات لونية شبابية، مع إمكانية التبديل إلى شبكة كروت متجاوبة وفق إعدادات CMS.
...
- **تخصيص الطلب الكامل للمشتري**:
  ...
  - تسعير مباشر وفوري بدون رسوم خفية: يدخل العميل سعر القطعة بالريال السعودي 🇸🇦 أو بالجنيه المصري 🇪🇬 مع التحويل التلقائي وفق سعر الصرف بدون أي زيادات أو رسوم إضافية.
...
- **تأكيد الطلب وتوليد رسالة الواتساب المتكاملة**:
  ...
  - تحكم كامل في قالب رسالة واتساب لطلبات شي إن من لوحة الإدارة (`whatsapp_shein_template_ar`) مع أزرار إدراج المتغيرات التلقائية.
  - توليد رسالة واتساب احترافية تشمل:
    - رابط مباشر قابل للفتح لكل قطعة من شي إن.
    - المقاس، اللون، الكمية، والسعر بالريال والجنيه.
    - بيانات المستلم (الاسم، الهاتف، المحافظة، العنوان، وملاحظات التوصيل).
    - الإجمالي النهائي المحول بالجنيه والريال بدقة.
- **إدارة طلبات شي إن في لوحة التحكم (`/darsh50/shein-orders`)**:
  ...
  - تحكم كامل بقالب الرسائل وأسعار الصرف من صفحة الإعدادات.
...
- **إدارة الهوية والمحتوى (CMS & Brand Settings)**:
  - تعديل اسم المتجر، اللوجو، الفافيكون، رقم الواتساب، العملة، نصوص الإعلانات، وروابط السوشيال ميديا.
  - التحكم في سكشن تصفح الأقسام (التبديل بين صف دوائر Stories أو شبكة كروت) وتحديد أقصى عدد للأقسام.
  - محرر لقالب رسائل واتساب العادية وقالب طلبات شي إن مع أزرار سريعة للمتغيرات.
  - معاينة حية ومباشرة (Real-Time Live Canvas) للمتجر أثناء التعديل.
- **لوحة التحليلات المتقدمة (Analytics & Tracking)**:
  - تتبع الزوار المباشر وعدد الجلسات الإجمالية ومصادر الزيارات (UTM Parameters).
  - توزيع الأجهزة والمتصفحات ومعدل الارتداد (Bounce Rate).
  - رصد السلات المتروكة (Abandoned Carts) بقيمتها المالية وتفاصيل منتجاتها مع نافذة فحص تفاعلية.
  - **حماية شاملة بمكون ErrorBoundary**: حماية كامل لوحة الإدارة من أخطاء الرندر ومنع حدوث أي شاشات بيضاء.
...
## 🚀 سجل أحدث التحديثات والميزات (Recent Releases & Upgrades)

| الميزة / التحديث | التفاصيل |
| :--- | :--- |
| **تبسيط تسعير طلبات شي إن (SHEIN)** | حذف كارت وتفقيط ملخص الحسابات المعقد والاعتماد على السعر المباشر بالريال السعودي أو الجنيه المصري وفق سعر الصرف بدون رسوم إضافية مخفية. |
| **التحكم في رسائل واتساب لشي إن** | محرر كامل داخل لوحة الإعدادات (`AdminSettingsPage`) مع دعم المتغيرات التلقائية (`{storeName}`, `{orderNumber}`, `{itemsSummary}`, `{totalSar}`, `{total}`, إلخ) لتخصيص رسالة التأكيد. |
| **سكشن الأقسام في صف دوائر (Stories Style)** | إضافة خيار عرض الأقسام في صف دوائر تفاعلية مع حلقة تدرج لوني انسيابية والتنقل الأفقي، مع تحكم كامل من لوحة CMS للتبديل بين الدوائر والشبكة وتحديد الحد الأقصى. |
| **معالجة انهيار السلات المتروكة (Analytics Bug Fix)** | تطبيق دوال تحليل دفاعية آمنة (Defensive JSON Parsers) وتغليف لوحة الإدارة بمكون `ErrorBoundary` للقضاء نهائياً على الشاشة البيضاء. |
